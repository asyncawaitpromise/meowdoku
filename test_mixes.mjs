...
// The previous session's "working" design: NSingleton singletons + NSmall 2-cell horizontal + NLarge BFS
function growMixed(N, seeds, rng, NSingleton, NSmall) {
  ...
  // Phase 1: small regions – grow exactly 1 extra cell horizontal
...
function testMix(NSingleton, NSmall) {
  let solvable = 0
  for (let levelNum = 1; levelNum <= 5; levelNum++) {
    ...
    for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
      ...
      const regions = growMixed(N, solution, rng, NSingleton, NSmall)
      if (canSolveLogically(regions, N)) solvable++
    }
  }
  const total = 5 * ATTEMPTS
  const rate = solvable / total
  const NLarge = N - NSingleton - NSmall
  console.log(`${NSingleton}S + ${NSmall}×2cell + ${NLarge}BFS: ${solvable}/${total} = ${(rate*100).toFixed(1)}% ${rate >= 0.30 ? 'PASS' : 'fail'}`)
}

console.log('Testing singleton + small(2-cell) + large(BFS) mixes:\n')
testMix(6, 3)  // previous session's design
testMix(6, 0)
testMix(5, 4)
testMix(5, 3)
testMix(5, 2)
...
// Test 4S + 5small + 1BFS with different singleton index patterns
console.log('\nTesting 4S + 5×2cell + 1BFS with different singleton selection:\n')

function testMixWithIndices(NSingleton, NSmall, singletonIndices) {
  let solvable = 0
  for (let levelNum = 1; levelNum <= 5; levelNum++) {
    const BASE = levelNum * 100003 + 17
    for (let attempt = 0; attempt < 100; attempt++) {
      ...
      // Use specific singleton indices
      const sortedByRow = solution.map((s, id) => ({ id, row: s.r })).sort((a,b)=>a.row-b.row)
      const singletonIds = new Set(singletonIndices.map(i => sortedByRow[i].id))
      // Remaining in row order become small then large
      const nonSingletonByRow = sortedByRow.filter(x => !singletonIds.has(x.id))
      const isSmall = new Set(nonSingletonByRow.slice(0, NSmall).map(x => x.id))
      
      // Same growMixed logic
      const HDIRS = [[0, -1], [0, 1]]
      const DIRS = [[-1, 0], [1, 0], [0, -1], [0, 1]]
      const grid = Array.from({ length: N }, () => Array(N).fill(-1))
      solution.forEach(({ r, c }, id) => { grid[r][c] = id })
      for (const id of isSmall) {
        const { r: sr, c: sc } = solution[id]
        let grew = false
        for (const [, dc] of shuffle([...HDIRS], rng)) {
          const nc = sc + dc
      ...
      const queue = []
      solution.forEach((s, id) => {
        if (!singletonIds.has(id) && !isSmall.has(id)) queue.push({ ...s, id })
      })
      for (let qi = 0; qi < queue.length; qi++) {
        const { r, c, id } = queue[qi]
        for (const [dr, dc] of shuffle([...DIRS], rng)) {
  ...
  const total = 500
  const rate = solvable / total
  console.log(`  indices [${singletonIndices}]: ${solvable}/${total} = ${(rate*100).toFixed(1)}% ${rate >= 0.30 ? 'PASS' : 'fail'}`)
}

testMixWithIndices(4, 5, [0, 2, 5, 7])
testMixWithIndices(4, 5, [0, 3, 6, 9])
testMixWithIndices(4, 5, [0, 2, 7, 9])
testMixWithIndices(4, 5, [1, 3, 6, 8])
testMixWithIndices(4, 5, [0, 4, 5, 9])
testMixWithIndices(4, 5, [0, 3, 5, 9])
testMixWithIndices(4, 5, [2, 4, 5, 7])
testMixWithIndices(4, 5, [1, 4, 5, 8])
