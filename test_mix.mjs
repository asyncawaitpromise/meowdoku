// Test: mixed singleton + small + large regions
// Try different numbers of singletons to find the threshold for 50%+
...
// Region generator: nSingleton singletons + nSmall small (2-3 cells) + rest large BFS
// nSingleton + nSmall + nLarge = N = 10
function growRegionsMix(N, seeds, rng, nSingleton, nSmall, smallTarget) {
  // Sort by column, first nSingleton → singleton, next nSmall → small, rest → large
  const sortedByCol = seeds.map((s, id) => ({ id, col: s.c })).sort((a, b) => a.col - b.col)
  const isSingleton = new Set()
  const isSmall = new Set()
  sortedByCol.forEach(({ id }, idx) => {
  ...
  // Singletons: already done (just 1 cell)

  // Small regions: grow to smallTarget cells using BFS
  ...
  // Large regions: BFS fill (interleaved, no cap)
  const largeQ = []
  ...
  // Cleanup: unclaimed → nearest non-singleton
...
const configs = [
  { nS: 5, nSm: 0, nL: 5, st: 4, label: '5sing+5large' },
  { nS: 5, nSm: 3, nL: 2, st: 3, label: '5sing+3small(3)+2large' },
  { nS: 5, nSm: 3, nL: 2, st: 2, label: '5sing+3small(2)+2large' },
  { nS: 6, nSm: 0, nL: 4, st: 4, label: '6sing+4large' },
  { nS: 7, nSm: 0, nL: 3, st: 4, label: '7sing+3large' },
  { nS: 7, nSm: 2, nL: 1, st: 2, label: '7sing+2small(2)+1large' },
  { nS: 8, nSm: 0, nL: 2, st: 4, label: '8sing+2large' },
  { nS: 8, nSm: 2, nL: 0, st: 4, label: '8sing+2small(4)' },
  { nS: 9, nSm: 0, nL: 1, st: 4, label: '9sing+1large' },
  { nS: 9, nSm: 1, nL: 0, st: 4, label: '9sing+1small(4)' },
]
...
for (const { nS, nSm, nL, st, label } of configs) {
  ...
  for (let level = 1; level <= 5; level++) {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const grid = growRegionsMix(N, solution, rng, nS, nSm, st)
      if (canSolveLogically(grid, N)) total++
    }
  }
  console.log(`${label}: ${total}/1000 (${(total/10).toFixed(1)}%)`)
}
