// Test different singleton selection strategies
...
// Try sorting by row (top first = row 0, 1, 2... become singletons)
function growRegionsByRow(N, seeds, rng, nSingleton, nSmall, smallTarget) {
  // Sort by row index ascending → first nSingleton rows become singletons
...
const configs = [
  { nS: 5, nSm: 4, nL: 1, st: 2 },
  { nS: 6, nSm: 3, nL: 1, st: 2 },
  { nS: 6, nSm: 2, nL: 2, st: 2 },
  { nS: 7, nSm: 2, nL: 1, st: 2 },
  { nS: 7, nSm: 1, nL: 2, st: 2 },
]

for (const { nS, nSm, nL, st } of configs) {
  let total = 0
  for (let level = 1; level <= 5; level++) {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const grid = growRegionsByRow(N, solution, rng, nS, nSm, st)
      if (canSolveLogically(grid, N)) total++
    }
  }
  console.log(`ROW ${nS}s+${nSm}sm(${st})+${nL}L: ${total}/1000 (${(total/10).toFixed(1)}%) ${total>=500?'✓':''}`)
}
