// Test: horizontal small regions (cat + 1 cell to left or right)
// Makes small regions same-row, which helps with row-based eliminations
...
// Horizontal small: 2 cells in the same row
// Vertical small: 2 cells in the same column
// Diagonal small: 2 cells in different row and col (BFS)
function growRegionsDirectional(N, seeds, rng, nSingleton, nSmall, sortMode, smallDir) {
  let sorted
  if (sortMode === 'row') {
    sorted = seeds.map((s, id) => ({ id, key: s.r })).sort((a, b) => a.key - b.key)
  } else if (sortMode === 'edge') {
    sorted = seeds.map((s, id) => ({ id, key: Math.min(s.r, N-1-s.r) + Math.min(s.c, N-1-s.c) }))
      .sort((a, b) => a.key - b.key)
  } else {
    sorted = seeds.map((s, id) => ({ id, key: s.c })).sort((a, b) => a.key - b.key)
  }
  ...
  const HDIRS = [[0,-1],[0,1]]  // horizontal only
  const VDIRS = [[-1,0],[1,0]]  // vertical only
  const ADIRS = [[-1,0],[1,0],[0,-1],[0,1]]  // all

  for (const id of isSmall) {
    const { r: sr, c: sc } = seeds[id]
    const dirs = smallDir === 'H' ? HDIRS : smallDir === 'V' ? VDIRS : shuffle([...ADIRS], rng)
    // Try to grow by 1 cell in preferred direction
    let grew = false
    ...
    // If couldn't grow in preferred dir, try all dirs
    if (!grew) {
      for (const [dr, dc] of shuffle([...ADIRS], rng)) {
        const nr = sr+dr, nc = sc+dc
        if (nr >= 0 && nr < N && nc >= 0 && nc < N && grid[nr][nc] === -1) {
          grid[nr][nc] = id; break
        }
...
const configs = [
  { nS: 7, nSm: 2, nL: 1, sort: 'row', dir: 'H', label: '7s+2sm-H+1L ROW' },
  { nS: 7, nSm: 2, nL: 1, sort: 'row', dir: 'V', label: '7s+2sm-V+1L ROW' },
  { nS: 7, nSm: 2, nL: 1, sort: 'row', dir: 'A', label: '7s+2sm-A+1L ROW' },
  { nS: 6, nSm: 3, nL: 1, sort: 'row', dir: 'H', label: '6s+3sm-H+1L ROW' },
  { nS: 6, nSm: 3, nL: 1, sort: 'row', dir: 'V', label: '6s+3sm-V+1L ROW' },
  { nS: 6, nSm: 3, nL: 1, sort: 'row', dir: 'A', label: '6s+3sm-A+1L ROW' },
  { nS: 6, nSm: 3, nL: 1, sort: 'edge', dir: 'H', label: '6s+3sm-H+1L EDGE' },
  { nS: 6, nSm: 3, nL: 1, sort: 'edge', dir: 'V', label: '6s+3sm-V+1L EDGE' },
  { nS: 6, nSm: 3, nL: 1, sort: 'edge', dir: 'A', label: '6s+3sm-A+1L EDGE' },
  { nS: 5, nSm: 4, nL: 1, sort: 'row', dir: 'H', label: '5s+4sm-H+1L ROW' },
  { nS: 5, nSm: 4, nL: 1, sort: 'edge', dir: 'H', label: '5s+4sm-H+1L EDGE' },
]

for (const { nS, nSm, nL, sort, dir, label } of configs) {
  let total = 0
  for (let level = 1; level <= 5; level++) {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const grid = growRegionsDirectional(N, solution, rng, nS, nSm, sort, dir)
