...
function growRegions(N, seeds, rng, N_SINGLETON, N_SMALL) {
...
for (const [ns, nsmall] of [[4,5],[5,4],[6,3]]) {
  console.log(`\n${ns}S + ${nsmall}×2cell + 1BFS:`)
  let allPass = true
  for (let levelNum = 1; levelNum <= 5; levelNum++) {
    ...
    for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
      ...
      const regions = growRegions(N, solution, rng, ns, nsmall)
      if (canSolveLogically(regions, N)) solvable++
    }
    const rate = solvable / ATTEMPTS
    const pass = rate >= 0.30
    if (!pass) allPass = false
    console.log(`  Level ${levelNum}: ${solvable}/${ATTEMPTS} = ${(rate*100).toFixed(1)}% ${pass ? 'PASS' : 'FAIL'}`)
  }
  console.log(allPass ? '  All levels PASS' : '  Some levels FAIL')
}
