...
function growRegions(N, seeds, rng, smallTarget, numSmall) {
  const sortedByCol = seeds.map((s, id) => ({ id, col: s.c })).sort((a, b) => a.col - b.col)
  const smallIds = new Set(sortedByCol.slice(0, numSmall).map(x => x.id))
  ...
  // Grow small regions first (to their target)
  const smallQueues = seeds.map(({ r, c }, id) => smallIds.has(id) ? [{ r, c }] : [])
  let anyP = true
  while (anyP) {
    anyP = false
    for (const id of smallIds) {
      if (sizes[id] >= smallTarget) continue
      const q = smallQueues[id]; if (q.length === 0) continue; anyP = true
      const idx = Math.floor(rng() * q.length); const { r, c } = q[idx]
      const dirs = shuffle([...DIRS], rng); let expanded = false
  ...
  // Grow large regions
  const queue = []
  seeds.forEach(({ r, c }, id) => { if (!smallIds.has(id)) queue.push({ r, c, id }) })
  ...
  for (let r = 0; r < N; r++) for (let c = 0; c < N; c++) {
    ...
    seeds.forEach(({ r: sr, c: sc }, id) => {
      if (smallIds.has(id)) return; const d = Math.abs(r - sr) + Math.abs(c - sc); if (d < bestDist) { bestDist = d; best = id }
    })
    grid[r][c] = best === -1 ? 0 : best
  }
  return grid
}
...
const N = 10
console.log('Testing (numSmall, smallTarget):')
for (const [numSmall, smallTarget] of [[5,1],[5,2],[5,3],[6,1],[6,2],[7,1],[7,2],[8,1],[8,2]]) {
  let total = 0
  for (let level = 1; level <= 5; level++) {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const regions = growRegions(N, solution, rng, smallTarget, numSmall)
      if (canSolveLogically(regions, N)) total++
    }
  }
  console.log(`  numSmall=${numSmall}, target=${smallTarget}: ${total}/1000 (${(total/10).toFixed(1)}%)`)
}
