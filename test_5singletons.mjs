...
// 5 singleton regions (first 5 by column), 5 large BFS regions
function growRegions(N, seeds, rng) {
  const sortedByCol = seeds.map((s, id) => ({ id, col: s.c })).sort((a, b) => a.col - b.col)
  const singletonIds = new Set(sortedByCol.slice(0, 5).map(x => x.id))

  const grid = Array.from({ length: N }, () => Array(N).fill(-1))
  seeds.forEach(({ r, c }, id) => { grid[r][c] = id })  // seed all

  const DIRS = [[-1, 0], [1, 0], [0, -1], [0, 1]]
  const queue = []
  seeds.forEach(({ r, c }, id) => {
    if (!singletonIds.has(id)) queue.push({ r, c, id })
  })
