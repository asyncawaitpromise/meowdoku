...
function growRegions(N, seeds, rng) {
  ...
  for (const id of isSmall) {
    ...
    while (sizes[id] < SMALL_TARGET && q.length > 0) {
      ...
      const c = q[idx]; let expanded = false
...
cands.forEach((cs, id) => {
  ...
  const cols2 = [...new Set(cs.map(COL))].sort((a,b)=>a-b)
  console.log(`  Reg ${id} (${isSmall.has(id)?'S':'L'}, ${sizes[id]}c): rows=${JSON.stringify(rows)} cols=${JSON.stringify(cols2)}`)
})
