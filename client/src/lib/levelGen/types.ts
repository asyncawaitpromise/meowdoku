export interface GeneratedLevel {
  ...
  techniqueCounts: Record<string, number> // per-technique elimination counts keyed by name (see STRATEGY_NAMES), e.g. { singleton: 12, "common-neighbor": 3 } — how often each skill fired while solving this puzzle
...
export interface SolveResult {
  solved: boolean; strategiesUsed: number; unsolvedCount: number; easySteps: number; hardSteps: number
  rounds: number; unsolvedRegions: number[]; maxSubsetSize: number
  // Per-technique elimination counts keyed by name (see STRATEGY_NAMES). Only
  // canSolveLogically populates this — canSolveFast is a hot-path heuristic
  // used thousands of times per generation and skips the bookkeeping.
