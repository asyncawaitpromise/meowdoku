...
// ORIGINAL generator (uniform BFS)
...
const N = 10
let found = 0
for (let seed = 0; seed < 10000 && found < 5; seed++) {
  const rng = makeRng(seed)
  const catCols = findPlacement(N, rng)
  const solution = catCols.map((c, r) => ({ r, c }))
  const regions = growOriginal(N, solution, rng)
  if (canSolveLogically(regions, N)) {
    console.log(`SOLVABLE at seed ${seed}! Cats: ${catCols}`)
    found++
  }
}
console.log(`Found ${found} solvable out of 10000 attempts with ORIGINAL generator`)
