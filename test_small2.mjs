// Test: 2-cell small regions (cat + 1 adjacent) + large BFS for the rest
// Also test 3-cell small regions
...
// Small regions: SMALL_TARGET cells (connected, grown from cat seed)
// Large regions: BFS fill from remaining seeds
function growRegionsSmallLarge(N, seeds, rng, SMALL_TARGET) {
  ...
  const largeQ = []
  seeds.forEach(({ r, c }, id) => { if (!isSmall.has(id)) largeQ.push({ r, c, id }) })
  let lqi = 0
  // Simple BFS from all large seeds simultaneously
  const q = [...largeQ]
  let qi = 0
  while (qi < q.length) {
    const { r, c, id } = q[qi++]
...
for (const SMALL_TARGET of [1, 2, 3, 4, 5, 6]) {
  let totalSolvable = 0
  for (let level = 1; level <= 5; level++) {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const grid = growRegionsSmallLarge(N, solution, rng, SMALL_TARGET)
      if (canSolveLogically(grid, N)) solvable++
    }
    totalSolvable += solvable
  }
  console.log(`SMALL_TARGET=${SMALL_TARGET}: ${totalSolvable}/1000 (${(totalSolvable/10).toFixed(1)}%)`)
}
