...
function growRegions(N, seeds, rng, numSingletons) {
  const sortedByCol = seeds.map((s, id) => ({ id, col: s.c })).sort((a, b) => a.col - b.col)
  const singletonIds = new Set(sortedByCol.slice(0, numSingletons).map(x => x.id))
  ...
  const queue = []
  seeds.forEach(({ r, c }, id) => { if (!singletonIds.has(id)) queue.push({ r, c, id }) })
...
const N = 10
for (const numSingletons of [1, 2, 3, 4, 5, 6, 7, 8, 9]) {
  let total = 0
  for (let level = 1; level <= 5; level++) {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const regions = growRegions(N, solution, rng, numSingletons)
      if (canSolveLogically(regions, N)) total++
    }
  }
  console.log(`${numSingletons} singletons: ${total}/1000 (${(total/10).toFixed(1)}%)`)
}
